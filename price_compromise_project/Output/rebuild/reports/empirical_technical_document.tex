...
\title{\heiti 价格型阶级妥协的脆弱性：技术附录\\
\large 数据来源、清洗规则、变量构造与计量模型}
\author{}
\date{}
...
本研究检验美国价格型阶级妥协的部门边界及其在外部冲击下的脆弱性。理论命题是：低价供给主要集中在可贸易消费品和可由全球供应链组织生产的商品上；对于住房、医疗、教育、护理、养老和育儿等高度依赖本地制度安排、公共服务体系、资产价格结构和本地劳动投入的再生产成本，低价商品渠道的缓冲能力有限。因此，价格型阶级妥协不是完整的再生产保障机制，而是一种局部、阶段性的稳定机制。当贸易品价格和供应链成本在疫情后快速上升时，这一稳定机制会暴露出脆弱性。
...
复现目录以 \path{price_compromise_project} 为核心。主要脚本和输出如下：
...
\path{Output/rebuild/reports/formal_empirical_chapter.tex} & 实证报告 LaTeX 源文件。 \\
\path{Output/rebuild/reports/empirical_technical_document.tex} & 技术附录 LaTeX 源文件。 \\
\bottomrule
\end{longtable}
...
\item 使用 XeLaTeX 编译实证报告和技术附录。
\end{enumerate}

主回归样本为 2017--2025 年、66 个 BEA 行业，共 594 个行业--年份观测。价格--工资压力缺口模型因工资数据缺失略少，样本为 559 个行业--年份观测。扩展样本覆盖 2010--2025 年、66 个 BEA 行业，共 1056 个行业--年份观测，用于稳健性检验而非替代主样本。

\section{数据来源}

\subsection{消费价格数据}

消费价格部分使用 BLS CPI-U 分类价格数据。清洗后的年度分类价格数据位于 \path{data/processed/cpi_annual_normalized.csv}。该文件已经将 CPI 子项转换为年度频率，并统一标准化为 1984 年等于 100。研究同时使用 \path{data/processed/constructed_price_indices.csv} 中的研究性指数，包括低价可贸易商品指数、再生产成本平减指数、本地化再生产成本指数和 CPI All Items 基准。
...
贸易品通胀代理来自 BLS CPI-U 中 ``commodities less food and energy commodities'' 的年度同比通胀率；非贸易品通胀代理来自 ``services less energy services''。主样本处理后的年度文件位于 \path{Data/analysis} 目录，重建后输出到 \path{Data/rebuild} 目录。扩展样本通过 BLS API 分段拉取 2009--2025 年 CPI 月度序列，并在 \path{Data/extended} 中输出 2010--2025 年年度通胀文件。
...
行业面板来自 BEA 年度行业数据和 QCEW 行业工资数据。面板包括 BEA summary 行业代码、行业名称、总产出价格指数、中间投入价格指数、PCE 价格指数、行业产出、QCEW 就业和工资变量。
...
投入产出网络来自 2019 年 BEA 投入系数矩阵。对应数据文件为 \path{bea_input_coefficients_2019.csv}。选择 2019 年矩阵有两个原因。第一，它位于 2021--2022 年通胀爆发之前，可以被视为相对预定的行业网络结构。第二，2019 年比 2020 年更少受到疫情停摆和临时性供需扰动影响。
...
Section 301 关税暴露由关税清单、商品分类和 BEA 行业映射表整理得到，并分别聚合到 CPI 类别和 BEA 行业层面。CPI 类别层面的暴露刻画关税冲击在商品部门的集中程度；BEA 行业层面的直接暴露和网络暴露进入政策冲击回归。本文不把 301 关税解释为 2021--2022 年通胀爆发的唯一原因，而把它作为低价商品供给机制受到政策扰动的补充证据。
...
\end{equation}
基准图表使用 \(\omega_{kg}=1/N_g\) 的未加权口径。这样处理的目的，是展示不同价格形成机制之间的结构性差异，而不是估计某个具体家庭的真实预算成本。这个设定有清楚限制：它没有使用收入组、家庭结构或地区差异的消费权重，因此不能被解释为官方生活成本指数。

更理想的下一步，是使用 CEX 或 PCE 支出权重构造加权版本：
\begin{equation}
Index^{W}_{gt}=\sum_{k\in g}s_{kg}P^{1984=100}_{kt},
\end{equation}
其中 \(s_{kg}\) 是消费支出权重。加权指数可以作为稳健性检验，用于比较未加权口径和支出权重口径是否支持相同的部门边界结论。
...
\end{equation}
基准图表使用 \(\omega^R_k=1/|\mathcal R|\) 的未加权设定。该指数只作为工资购买力背景平减口径，不作为主回归结果变量。
...
扩展样本的清洗规则与主样本保持一致，但有三点单独说明。第一，为计算 2010 年价格增长率，扩展脚本从 BEA 原始价格表中额外读取 2009 年价格指数作为滞后项，最终回归面板仍从 2010 年开始。第二，BLS QCEW 年度 area API 对 2009--2013 年返回 404；考虑到扩展检验的核心结果变量是 BEA 价格增长率，扩展样本不下载每年约 130MB 的历史压缩包，只保留 2014--2025 年可由 API 直接取得的 QCEW 工资数据。因此扩展样本的价格方程完整覆盖 2010--2025 年，工资缺口变量不作为扩展样本主结果。第三，扩展样本仍使用 2019 年投入产出矩阵构造暴露，因此它应被解释为稳健性检验，而不是替代主样本的历史网络设计。
...
模型的核心识别假设是：2019 年投入产出暴露可以被视为 2021--2022 年价格冲击之前的预定行业结构。行业固定效应控制长期不变的行业特征，年份固定效应控制共同宏观通胀、货币政策、总需求变化和疫情期间的全国性冲击。因此，估计系数主要来自同一年度内不同行业因预定网络暴露不同而出现的价格增长差异。
...
该实证设计已经形成可解释、可复现的行业价格传导框架，但仍存在若干限制。第一，再生产成本指数采用基准未加权口径，尚未纳入 CEX 或 PCE 支出权重。第二，行业网络暴露使用 2019 年投入产出结构，适合解释疫情前后的预定网络位置，但不能完全刻画较长历史时期内网络结构的变化。第三，行业价格传导并不等同于家庭层面的生活成本危机；住房、医疗、育儿、养老和教育等成本还需要地区层面和家庭支出层面的数据进一步识别。第四，贸易品通胀冲击可以与全球供应链压力指数、运输成本、库存调整和货币政策冲击共同建模，以进一步区分不同宏观冲击来源。
